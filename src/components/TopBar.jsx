...
function TopBar({
  ...
  const [activeSubTab, setActiveSubTab] = useState('Current Sprint');
  ...
  return (
    <header className={`flex justify-between items-center px-8 h-16 bg-background/80 backdrop-blur-md sticky top-0 z-20 border-b border-outline-variant transition-all duration-300 ease-in-out ${isSidebarOpen ? 'ml-64 w-[calc(100%-16rem)]' : 'ml-0 w-full'}`}>
      <div className="flex items-center flex-grow max-w-xl gap-3">
        ...
        {activeTab !== 'Standups' && (
          <div className="relative w-full">
            ...
            <input 
              ...
              placeholder={activeTab === 'History' ? 'Search team logs...' : 'Search workspace...'} 
            />
            {searchQuery && (
        ...
        {activeTab === 'Standups' && (
          <div className="flex items-center gap-6 text-sm font-semibold text-on-surface-variant ml-2 mt-1">
            <button 
              onClick={() => setActiveSubTab('Current Sprint')} 
              className={`pb-1.5 border-b-2 transition-colors cursor-pointer ${activeSubTab === 'Current Sprint' ? 'border-[#4f7c9e] text-[#4f7c9e]' : 'border-transparent hover:text-on-surface'}`}
            >
              Current Sprint
            </button>
            <button 
              onClick={() => setActiveSubTab('Team Health')} 
              className={`pb-1.5 border-b-2 transition-colors cursor-pointer ${activeSubTab === 'Team Health' ? 'border-[#4f7c9e] text-[#4f7c9e]' : 'border-transparent hover:text-on-surface'}`}
            >
              Team Health
            </button>
          </div>
      ...
      <div className={`flex items-center ${activeTab === 'Standups' ? 'gap-5' : 'gap-4'}`}>
        {activeTab === 'Standups' && (
          <button onClick={() => alert('Search feature coming soon!')} className="p-2 text-on-surface-variant hover:text-on-surface cursor-pointer rounded-lg hover:bg-surface-container-highest transition-colors">
        ...
        {activeTab === 'Standups' ? (
          <>
            <img onClick={() => alert('Profile clicked')} src="https://i.pravatar.cc/150?u=alexchen" alt="Alex Chen" className="w-9 h-9 rounded-full border border-outline-variant/60 cursor-pointer shadow-sm hover:ring-2 hover:ring-primary/30 transition-all" />
          </>
        ) : (
          <>
            <div className="relative">
            ...
            <img onClick={() => alert('Profile clicked')} src="https://i.pravatar.cc/150?u=alexchen" alt="Alex Chen" className="w-9 h-9 rounded-full border border-outline-variant/60 cursor-pointer shadow-sm hover:ring-2 hover:ring-primary/30 transition-all ml-2" />
          </>
        )}
      </div>
    </header>
